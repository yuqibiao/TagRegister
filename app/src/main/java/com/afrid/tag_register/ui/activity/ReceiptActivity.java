...
import com.afrid.tag_register.adapter.LinenAdapter;
import com.afrid.tag_register.bean.SubReceiptListBean;
import com.afrid.tag_register.bean.json.BaseJsonResult;
import com.afrid.tag_register.bean.json.SaveReceiptRequest;
...
import com.afrid.tag_register.utils.PrintStrBuildUtils;
...
public class ReceiptActivity extends MyBaseActivity {
    ...
    //是否为一体机
    private boolean isZKC = false;

    @Override
    public void beforeInit() {
        super.beforeInit();
        zkcManager = ZKCManager.getInstance(this);
        float zkc_holder = getResources().getDimension(R.dimen.zkc_holder);
        if (zkc_holder>0){
            isZKC = true;
        }else{
            isZKC = false;
        ...
        tagInfoListSubscription = apiMethodManager.getTagInfoList(getTagInfoRequest, new IRequestCallback<GetTagInfoListReturn>() {
            ...
            public void onSuccess(GetTagInfoListReturn result) {
                ...
                if (resultCode == 200) {
                    ...
                    adapter.setmData(mIxTagLinenList);
    ...
    public void saveReceipt(View view) {
        ...
        requestDataBean.setReceiptWarehouseId(application.getCheckedWashFactoryId());
        requestDataBean.setSenderWarehouseId(application.getCheckedSupplierId());
        requestDataBean.setSubReceiptList(resultData.getMIxTagLinenList());
        requestDataBean.setTrainCode(application.getCheckedTrainCode());
        requestDataBean.setWarehouseId(application.getCheckedLinenTypeId()+"");
            ...
            public void onSuccess(BaseJsonResult<String> result) {
                ...
                if (resultCode == 200) {
                    ...
                    if (isZKC){
                        ...
                        MyToast.showShort(ReceiptActivity.this, resourceUtils.getStr(R.string.receipt_submit_success));
                        finish();
                    }
